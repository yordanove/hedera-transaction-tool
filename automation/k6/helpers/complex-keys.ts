...
/**
 * Get the minimum number of signatures needed to satisfy a complex key
 *
 * For Hedera-style (17 of 29 with 1-of-N children):
 * - Need 17 child thresholds satisfied
 * - Each child needs 1 signature
 * - So minimum = 17 signatures
 *
 * @param metadata - Key metadata from generation
 * @returns Minimum signatures required
 */
export function getMinimumSignaturesRequired(metadata: ComplexKeyMetadata): number {
  // Each child needs threshold signatures, parent needs parentThreshold children
  // For 1-of-N children, each child needs 1 signature
  // So total = parentThreshold * (typical child threshold, usually 1)
  const typicalChildThreshold = metadata.childConfigs[0]?.threshold ?? 1;
  return metadata.parentThreshold * typicalChildThreshold;
}

/**
 * Select the minimum set of private keys needed to satisfy the threshold
 *
 * @param result - ComplexKeyResult from key generation
 * @returns Array of private keys sufficient to sign (minimum set)
 */
export function selectMinimumSigningKeys(result: ComplexKeyResult): PrivateKey[] {
  const { metadata, allPrivateKeys } = result;
  const selectedKeys: PrivateKey[] = [];
  let keyIndex = 0;

  // Select one key from each of the first N children (where N = parentThreshold)
  for (let i = 0; i < metadata.parentThreshold && i < metadata.childCount; i++) {
    const childConfig = metadata.childConfigs[i];
    // Take the first key from this child's set
    if (keyIndex < allPrivateKeys.length) {
      selectedKeys.push(allPrivateKeys[keyIndex]);
    }
    // Skip to next child's keys
    keyIndex += childConfig.numKeys;
  }
