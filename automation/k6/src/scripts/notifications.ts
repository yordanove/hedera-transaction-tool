...
export default function (data: SetupData): void {
  ...
  group('Notifications Page', () => {
    ...
    check(res, {
      ...
      'notifications response < 1s': (r) => r.timings.duration < 1000,
    });

    console.log(`Notifications load time: ${formatDuration(res.timings.duration)}`);
  });
...
export function handleSummary(data: SummaryData): SummaryOutput {
  ...
  return {
    'k6/reports/notifications-report.html': htmlReport(data),
    'k6/reports/notifications.json': JSON.stringify(data, null, 2),
    stdout: textSummary(data, 'Notifications'),
  };
}
