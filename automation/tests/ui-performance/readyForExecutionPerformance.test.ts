/**
 * Performance Test: Ready for Execution Tab (Org Mode)
 *
 * Requirement: Tab loads in ≤ 1s
 * Data source: Backend PostgreSQL (seeded by k6:seed:all)
 *
 * Note: Ready for Execution tab shows transactions with status WAITING_FOR_EXECUTION
 * (fully signed, ready to submit to Hedera)
 *
 * Prerequisites:
...
test.describe('Ready for Execution Performance (Org Mode)', () => {
  test.beforeAll(async () => {
    ...
    await setupOrgModeTestEnvironment(window, registrationPage, organizationPage, 'perf-ready-for-execution');
  });
  ...
  test('Ready for Execution tab should load in under 1 second (p95)', async () => {
    ...
    await window.click(SELECTORS.TAB_READY_FOR_EXECUTION);
    await window.waitForLoadState('networkidle');

    await enforceVolumeRequirement(window, REQUIRED_TOTAL, 'Ready for Execution');
    ...
    console.log(
      `Ready for Execution p95: ${formatDuration(samples.p95)}, avg: ${formatDuration(samples.avg)}`,
    );
